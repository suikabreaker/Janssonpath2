...
// structures below are not visiable external. in interface there's only
// jsonpath_t*.

// we avoid to contain jsonpath_t* node here as $.a[1].tag is a really commonly
// seen pattern. straightforward implementation would make it like a linked list
// which causes indirect memory access at execution. this affects performance
// expecially for filter expression. instead we save it as an array.
...
typedef struct path_index_t {
    ...
    union {
        // NULL for *, JSON_NULL for #, minus for counting back, INT_MAX for
        // end(one pass the last) path_index_t is responsible to lifetime of
        // objects below
